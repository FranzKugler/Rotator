...
Api::Api(std::vector<Device *> &devices, const char *server_id, const char *server_name,
         const char *manufacturer, const char *manufacturer_version, const char *location)
{
...
void Api::initialize()
{
    for (auto it : _devices)
    {
        for (auto device : it.second)
        {
            ...
            esp_err_t err =
                generate_unique_id(_server_id, name, device->device_type(), device->_number,
                                   device->_unique_id, sizeof(device->_unique_id));
            ESP_ERROR_CHECK(err);
        }
    }
}

esp_err_t Api::generate_unique_id(const char *server_id, const char *device_name,
                                  DeviceType device_type, uint8_t device_number, char *unique_id,
                                  size_t len)
{
...
void Api::register_device_routes(httpd_handle_t server, size_t device_number, Device *device)
{
    ...
    REGISTER_DEVICE_ROUTE(device_type_str, "commandblind", device_number, HTTP_PUT,
                          put_commandblind);
    REGISTER_DEVICE_ROUTE(device_type_str, "commandbool", device_number, HTTP_PUT, put_commandbool);
    REGISTER_DEVICE_ROUTE(device_type_str, "commandstring", device_number, HTTP_PUT,
                          put_commandstring);
    ...
    REGISTER_DEVICE_ROUTE(device_type_str, "driverversion", device_number, HTTP_GET,
                          get_driverversion);
    REGISTER_DEVICE_ROUTE(device_type_str, "interfaceversion", device_number, HTTP_GET,
                          get_interfaceversion);
    REGISTER_DEVICE_ROUTE(device_type_str, "name", device_number, HTTP_GET, get_name);
    REGISTER_DEVICE_ROUTE(device_type_str, "supportedactions", device_number, HTTP_GET,
...
void Api::register_covercalibrator_routes(httpd_handle_t server, size_t device_number,
                                          CoverCalibrator *device)
{
    char uri[128];

    REGISTER_DEVICE_ROUTE("covercalibrator", "brightness", device_number, HTTP_GET,
                          get_covercalibrator_brightness);
    REGISTER_DEVICE_ROUTE("covercalibrator", "calibratorstate", device_number, HTTP_GET,
                          get_covercalibrator_calibratorstate);
    REGISTER_DEVICE_ROUTE("covercalibrator", "coverstate", device_number, HTTP_GET,
                          get_covercalibrator_coverstate);
    REGISTER_DEVICE_ROUTE("covercalibrator", "maxbrightness", device_number, HTTP_GET,
                          get_covercalibrator_maxbrightness);
    REGISTER_DEVICE_ROUTE("covercalibrator", "calibratoroff", device_number, HTTP_PUT,
                          put_covercalibrator_calibratoroff);
    REGISTER_DEVICE_ROUTE("covercalibrator", "calibratoron", device_number, HTTP_PUT,
                          put_covercalibrator_calibratoron);
    REGISTER_DEVICE_ROUTE("covercalibrator", "closecover", device_number, HTTP_PUT,
                          put_covercalibrator_closecover);
    REGISTER_DEVICE_ROUTE("covercalibrator", "haltcover", device_number, HTTP_PUT,
                          put_covercalibrator_haltcover);
    REGISTER_DEVICE_ROUTE("covercalibrator", "opencover", device_number, HTTP_PUT,
                          put_covercalibrator_opencover);
}
...
void Api::register_dome_routes(httpd_handle_t server, size_t device_number, Dome *device)
{
    ...
    REGISTER_DEVICE_ROUTE("dome", "cansetaltitude", device_number, HTTP_GET,
                          get_dome_cansetaltitude);
    ...
    REGISTER_DEVICE_ROUTE("dome", "cansyncazimuth", device_number, HTTP_GET,
                          get_dome_cansyncazimuth);
    ...
    REGISTER_DEVICE_ROUTE("dome", "slewtoaltitude", device_number, HTTP_PUT,
                          put_dome_slewtoaltitude);
...
void Api::register_filterwheel_routes(httpd_handle_t server, size_t device_number,
                                      FilterWheel *device)
{
    char uri[128];

    REGISTER_DEVICE_ROUTE("filterwheel", "focusoffsets", device_number, HTTP_GET,
                          get_filterwheel_focusoffsets);
    REGISTER_DEVICE_ROUTE("filterwheel", "names", device_number, HTTP_GET, get_filterwheel_names);
    REGISTER_DEVICE_ROUTE("filterwheel", "position", device_number, HTTP_GET,
                          get_filterwheel_position);
    REGISTER_DEVICE_ROUTE("filterwheel", "position", device_number, HTTP_PUT,
                          put_filterwheel_position);
}
...
void Api::register_focuser_routes(httpd_handle_t server, size_t device_number, Focuser *device)
{
    ...
    REGISTER_DEVICE_ROUTE("focuser", "maxincrement", device_number, HTTP_GET,
                          get_focuser_maxincrement);
    ...
    REGISTER_DEVICE_ROUTE("focuser", "tempcompavailable", device_number, HTTP_GET,
                          get_focuser_tempcompavailable);
    REGISTER_DEVICE_ROUTE("focuser", "temperature", device_number, HTTP_GET,
...
void Api::register_observingconditions_routes(httpd_handle_t server, size_t device_number,
                                              ObservingConditions *device)
{
    char uri[128];

    REGISTER_DEVICE_ROUTE("observingconditions", "averageperiod", device_number, HTTP_GET,
                          get_observingconditions_averageperiod);
    REGISTER_DEVICE_ROUTE("observingconditions", "averageperiod", device_number, HTTP_PUT,
                          put_observingconditions_averageperiod);
    REGISTER_DEVICE_ROUTE("observingconditions", "cloudcover", device_number, HTTP_GET,
                          get_observingconditions_cloudcover);
    REGISTER_DEVICE_ROUTE("observingconditions", "dewpoint", device_number, HTTP_GET,
                          get_observingconditions_dewpoint);
    REGISTER_DEVICE_ROUTE("observingconditions", "humidity", device_number, HTTP_GET,
                          get_observingconditions_humidity);
    REGISTER_DEVICE_ROUTE("observingconditions", "pressure", device_number, HTTP_GET,
                          get_observingconditions_pressure);
    REGISTER_DEVICE_ROUTE("observingconditions", "rainrate", device_number, HTTP_GET,
                          get_observingconditions_rainrate);
    REGISTER_DEVICE_ROUTE("observingconditions", "skybrightness", device_number, HTTP_GET,
                          get_observingconditions_skybrightness);
    REGISTER_DEVICE_ROUTE("observingconditions", "skyquality", device_number, HTTP_GET,
                          get_observingconditions_skyquality);
    REGISTER_DEVICE_ROUTE("observingconditions", "skytemperature", device_number, HTTP_GET,
                          get_observingconditions_skytemperature);
    REGISTER_DEVICE_ROUTE("observingconditions", "starfwhm", device_number, HTTP_GET,
                          get_observingconditions_starfwhm);
    REGISTER_DEVICE_ROUTE("observingconditions", "temperature", device_number, HTTP_GET,
                          get_observingconditions_temperature);
    REGISTER_DEVICE_ROUTE("observingconditions", "winddirection", device_number, HTTP_GET,
                          get_observingconditions_winddirection);
    REGISTER_DEVICE_ROUTE("observingconditions", "windgust", device_number, HTTP_GET,
                          get_observingconditions_windgust);
    REGISTER_DEVICE_ROUTE("observingconditions", "windspeed", device_number, HTTP_GET,
                          get_observingconditions_windspeed);
    REGISTER_DEVICE_ROUTE("observingconditions", "refresh", device_number, HTTP_PUT,
                          put_observingconditions_refresh);
    REGISTER_DEVICE_ROUTE("observingconditions", "sensordescription", device_number, HTTP_GET,
                          get_observingconditions_sensordescription);
    REGISTER_DEVICE_ROUTE("observingconditions", "timesincelastupdate", device_number, HTTP_GET,
...
void Api::register_rotator_routes(httpd_handle_t server, size_t device_number, Rotator *device)
{
    ...
    REGISTER_DEVICE_ROUTE("rotator", "mechanicalposition", device_number, HTTP_GET,
                          get_rotator_mechanicalposition);
    ...
    REGISTER_DEVICE_ROUTE("rotator", "targetposition", device_number, HTTP_GET,
                          get_rotator_targetposition);
    ...
    REGISTER_DEVICE_ROUTE("rotator", "moveabsolute", device_number, HTTP_PUT,
                          put_rotator_moveabsolute);
    REGISTER_DEVICE_ROUTE("rotator", "movemechanical", device_number, HTTP_PUT,
                          put_rotator_movemechanical);
    REGISTER_DEVICE_ROUTE("rotator", "sync", device_number, HTTP_PUT, put_rotator_sync);
}

void Api::register_safetymonitor_routes(httpd_handle_t server, size_t device_number,
                                        SafetyMonitor *device)
{
    char uri[128];

    REGISTER_DEVICE_ROUTE("safetymonitor", "issafe", device_number, HTTP_GET,
                          get_safetymonitor_issafe);
}
...
void Api::register_switch_routes(httpd_handle_t server, size_t device_number, Switch *device)
{
    ...
    REGISTER_DEVICE_ROUTE("switch", "getswitchdescription", device_number, HTTP_GET,
                          get_switch_getswitchdescription);
    REGISTER_DEVICE_ROUTE("switch", "getswitchname", device_number, HTTP_GET,
                          get_switch_getswitchname);
    REGISTER_DEVICE_ROUTE("switch", "getswitchvalue", device_number, HTTP_GET,
                          get_switch_getswitchvalue);
    REGISTER_DEVICE_ROUTE("switch", "minswitchvalue", device_number, HTTP_GET,
                          get_switch_minswitchvalue);
    REGISTER_DEVICE_ROUTE("switch", "maxswitchvalue", device_number, HTTP_GET,
                          get_switch_maxswitchvalue);
    REGISTER_DEVICE_ROUTE("switch", "setswitch", device_number, HTTP_PUT, put_switch_setswitch);
    REGISTER_DEVICE_ROUTE("switch", "setswitchname", device_number, HTTP_PUT,
                          put_switch_setswitchname);
    REGISTER_DEVICE_ROUTE("switch", "setswitchvalue", device_number, HTTP_PUT,
                          put_switch_setswitchvalue);
...
esp_err_t Api::parse_request(httpd_req_t *req, alpaca_request_t *parsed_request)
{
    ...
    if (esp_log_level_get(TAG) >= ESP_LOG_DEBUG)
    {
        char debugstr[512] = {0};
        cJSON_bool ret =
            cJSON_PrintPreallocated(parsed_request->body, debugstr, sizeof(debugstr), false);
        if (!ret)
        {
...
esp_err_t Api::send_json_response(httpd_req_t *req, alpaca_request_t *parsed_request, cJSON *root,
                                  uint16_t status_code)
{
    ...
    if (esp_log_level_get(TAG) >= ESP_LOG_DEBUG)
    {
        ...
        ESP_LOGD(TAG, "%s %s %lldus %ld", method, req->uri, duration,
                 parsed_request->server_transaction_id);
    }
...
esp_err_t Api::handle_put_action(httpd_req_t *req)
{
    ...
    char *action =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Action"));
    char *parameters =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Parameters"));
...
esp_err_t Api::handle_put_commandblind(httpd_req_t *req)
{
    ...
    char *command =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Command"));
...
esp_err_t Api::handle_put_connected(httpd_req_t *req)
{
    ...
    char *connected =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Connected"));
...
esp_err_t Api::handle_put_covercalibrator_calibratoron(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::CoverCalibrator)
    {
        ...
        char *brightness = cJSON_GetStringValue(
            cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Brightness"));
        if (brightness)
        {
...
esp_err_t Api::handle_put_dome_slaved(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Dome)
    {
        ...
        char *slaved =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Slaved"));
...
esp_err_t Api::handle_put_dome_slewtoaltitude(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Dome)
    {
        ...
        char *altitude =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Altitude"));
        if (altitude)
        {
            char *endptr;
...
esp_err_t Api::handle_put_dome_slewtoazimuth(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Dome)
    {
        ...
        char *azimuth =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Azimuth"));
        if (azimuth)
        {
            char *endptr;
...
esp_err_t Api::handle_put_focuser_tempcomp(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Focuser)
    {
        ...
        char *tempcomp =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "TempComp"));
...
esp_err_t Api::handle_put_observingconditions_averageperiod(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::ObservingConditions)
    {
        ...
        char *averageperiod = cJSON_GetStringValue(
            cJSON_GetObjectItemCaseSensitive(parsed_request.body, "AveragePeriod"));
        if (averageperiod)
        {
            char *endptr;
...
esp_err_t Api::handle_get_observingconditions_sensordescription(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::ObservingConditions)
    {
        ...
        if (check_return(observingconditions->get_sensordescription(sensor_name->valuestring,
                                                                    sensordescription, 512),
                         root))
        {
...
esp_err_t Api::handle_get_switch_getswitchdescription(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Switch)
    {
        ...
        if (id)
        {
            ...
            if (check_return(
                    switch_device->get_getswitchdescription(id_value, switchdescription, 512),
                    root))
            {
...
esp_err_t Api::handle_put_switch_setswitch(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Switch)
    {
        ...
        char *id =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Id"));
        char *state =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "State"));
...
esp_err_t Api::handle_put_switch_setswitchname(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Switch)
    {
        ...
        char *id =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Id"));
        char *name =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Name"));
        if (id && name)
        {
            char *endptr;
            int32_t id_value = strtol(id, &endptr, 10);
...
esp_err_t Api::handle_put_switch_setswitchvalue(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Switch)
    {
        ...
        char *id =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Id"));
        char *value =
            cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(parsed_request.body, "Value"));
        if (id && value)
        {
            char *endptr;
            int32_t id_value = strtol(id, &endptr, 10);
...
esp_err_t Api::handle_put_telescope_declinationrate(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        double DeclinationRate =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "DeclinationRate"));
...
esp_err_t Api::handle_put_telescope_doesrefraction(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        {
            if (check_return(telescope_device->put_doesrefraction(cJSON_IsTrue(DoesRefraction)),
                             root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_guideratedeclination(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        {
            if (check_return(telescope_device->put_guideratedeclination(GuideRateDeclination),
                             root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_guideraterightascension(httpd_req_t *req)
{
    ...
    {
        Telescope *telescope_device = (Telescope *)device;
        double GuideRateRightAscension = cJSON_GetNumberValue(
            cJSON_GetObjectItem(parsed_request.body, "GuideRateRightAscension"));
        if (GuideRateRightAscension != NAN)
        {
            if (check_return(telescope_device->put_guideraterightascension(GuideRateRightAscension),
                             root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_rightascensionrate(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        double RightAscensionRate =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "RightAscensionRate"));
...
esp_err_t Api::handle_put_telescope_sideofpier(httpd_req_t *req)
{
    ...
    {
        Telescope *telescope_device = (Telescope *)device;
        Telescope::SideOfPier sideofpier = (Telescope::SideOfPier)cJSON_GetNumberValue(
            cJSON_GetObjectItem(parsed_request.body, "SideOfPier"));
...
esp_err_t Api::handle_put_telescope_siteelevation(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        double SiteElevation =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "SiteElevation"));
...
esp_err_t Api::handle_put_telescope_sitelatitude(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "SiteLatitude"));
        if (value != NAN)
        {
...
esp_err_t Api::handle_put_telescope_sitelongitude(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "SiteLongitude"));
        if (value != NAN)
        {
...
esp_err_t Api::handle_put_telescope_slewsettletime(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "SlewSettleTime"));
        if (value != NAN)
        {
...
esp_err_t Api::handle_put_telescope_targetdeclination(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "TargetDeclination"));
        if (value != NAN)
        {
...
esp_err_t Api::handle_put_telescope_targetrightascension(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "TargetRightAscension"));
        if (value != NAN)
        {
...
esp_err_t Api::handle_put_telescope_trackingrate(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        double value =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "TrackingRate"));
        if (value != NAN && value >= 0 && value <= 3)
        {
            if (check_return(telescope_device->put_trackingrate((Telescope::TrackingRate)value),
                             root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_utcdate(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        std::string value =
            cJSON_GetStringValue(cJSON_GetObjectItem(parsed_request.body, "UTCDate"));
        if (!value.empty())
        {
...
esp_err_t Api::handle_get_telescope_destinationsideofpier(httpd_req_t *req)
{
    ...
    {
        Telescope *telescope_device = (Telescope *)device;
        char *RightAscensionStr =
            cJSON_GetStringValue(cJSON_GetObjectItem(parsed_request.body, "RightAscension"));
        char *DeclinationStr =
            cJSON_GetStringValue(cJSON_GetObjectItem(parsed_request.body, "Declination"));
        if (RightAscensionStr != NULL && DeclinationStr != NULL)
        {
            ...
            if (check_return(telescope_device->get_destinationsideofpier(RightAscension,
                                                                         Declination, &value),
                             root))
            {
                cJSON_AddNumberToObject(root, "Value", (int)value);
...
esp_err_t Api::handle_put_telescope_moveaxis(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        if (Rate != NAN)
        {
            if (check_return(telescope_device->put_moveaxis((Telescope::TelescopeAxis)Axis, Rate),
                             root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_pulseguide(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        Telescope *telescope_device = (Telescope *)device;
        int32_t Direction =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "Direction"));
        int32_t Duration =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "Duration"));
        if (Direction != NAN && Duration != NAN)
        {
            if (check_return(telescope_device->put_pulseguide((Telescope::GuideDirection)Direction,
                                                              Duration),
                             root))
            {
            }
...
esp_err_t Api::handle_put_telescope_slewtoaltazasync(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        double Altitude =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "Altitude"));
...
esp_err_t Api::handle_put_telescope_slewtocoordinatesasync(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        double RightAscension =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "RightAscension"));
        double Declination =
            cJSON_GetNumberValue(cJSON_GetObjectItem(parsed_request.body, "Declination"));
        if (RightAscension != NAN && Declination != NAN)
        {
            if (check_return(
                    telescope_device->put_slewtocoordinatesasync(RightAscension, Declination),
                    root))
            {
            }
        }
...
esp_err_t Api::handle_put_telescope_synctocoordinates(httpd_req_t *req)
{
    ...
    if (parsed_request.device_type == DeviceType::Telescope)
    {
        ...
        if (RightAscension != NAN && Declination != NAN)
        {
            if (check_return(telescope_device->put_synctocoordinates(RightAscension, Declination),
                             root))
            {
            }
        }
